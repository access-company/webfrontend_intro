...
- [変数宣言](#変数宣言)
  ...
  - [var](#var)
- [関数](#関数)
  - [Allow function(ES2015)](#Allow-function(ES2015))
- [演算子](#演算子)
  - [基本的な二項演算子](#基本的な二項演算子)
  - [比較演算子](#比較演算子)
- [暗黙的な型変換](#暗黙的な型変換)
...
- 配列  
  `[` と `]` で値をカンマ区切りで囲み、その値を持つArrayオブジェクトを作成します
- 正規表現  
...
if文の条件式には `true` または `false` といった真偽値以外の値も指定できます。  
...
複数の条件分岐を書く場合は、if文に続けて `else if` 文を使うことで書けます。
...
JavaScriptは基本的にブラウザのメインスレッドで実行されるため、表示が更新されなくなりフリーズしたようになります。
...
`Promise` は、ES2015で導入された非同期処理を扱うビルトインオブジェクトです。  
...
ES2017では、 `Async Function` という非同期処理を行う関数を定義する構文が導入されました。  
