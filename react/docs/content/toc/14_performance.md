...
リアクティブな値には、props、state、コンポーネント本体に直線宣言された変数および関数が含まれます。
リアクティブな値ではない値の例として、コンポーネントの外で宣言された変数および関数や、useEffect,useCallback,useMemo の第1引数の関数内で定義された変数や関数が挙げられます。
