...
これらの副作用は、「ユーザーのアクションがトリガーとなる副作用」と「画面表示がトリガーとなる副作用」の2つに分類できます。
Reactの公式ドキュメントでは、特に後者のことを副作用(side effect)と区別するために「エフェクト(Effect)」と名付けています。そのため、この資料もそれに習って「画面表示がトリガーとなる副作用」をエフェクトと呼ぶことにします。

例えば、Submitボタンを押してサーバーに対してリクエストを送る動作は「ユーザーのアクションがトリガーとなる副作用」で、画面の初期表示時にサーバーからデータを取得する動作は「画面表示がトリガーとなる副作用」エフェクトです。

「ユーザーのアクションがトリガーとなる副作用」はイベントハンドラに書きます。

一方、エフェクトはどこに書けばよいでしょうか？
単にコンポーネント内に書いてしまうと、Reactでコンポーネントのルールである「関数の実行中に副作用が起きてはならない」というルールに違反してしまいます。
そこで、useEffectAPIを使います。

# useEffect API の導入

エフェクトをコンポーネントに含める場合は、`useEffect` Hooks API を使います。

エフェクトをコールバックとして記述します。そのエフェクトに依存する変数を
依存リストに含めます。
...
**React におけるクリーンナップ処理は、再レンダリング前とアンマウント前に実行されます。
...
ただ、ルールに従った結果、エフェクトが頻繁に実行されすぎたり、無限ループが発生したりすることがあります。そのような事象例や解決方法は、[公式ドキュメント エフェクトから依存値を取り除く](https://ja.react.dev/learn/removing-effect-dependencies)に書かれているので読んでおくとよいでしょう。

// useEffectをつかったデータフェッチのサンプルコードがあってもよいと思いました。
