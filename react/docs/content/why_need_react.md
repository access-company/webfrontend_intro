...
# 【課題0-1】vanillaJSでHTML/CSSを構築してみよう

以下のCardデザインイメージをvanillaJSで構築してください。

* 目的： DOM API を使ってみて、Reactを採用する理由を体感するため
* 制限： OSS、TypeScriptは利用しない
* デザインイメージ
  ![Card](./00_Card.png)

## Step 1: DOMツリーを構築する

以下のDOMツリーをDOM APIを使って構築してください（[Fork](https://codepen.io/aseijiurushihara/pen/WNQYYEQ) : TypeScriptは使いません）

なお、本課題では、`innerHTML` を使うことは禁止とします。HTMLコメント（`<!-- コメント -->`）の出力は不要です。
...
* id属性のElementオブジェクトを取得する（[MDN](https://developer.mozilla.org/ja/docs/Web/API/Document/getElementById)）
  ```javascript
  const element = document.getElementById(id)
  ```
* tagNameで指定したHTML要素を生成する（[MDN](https://developer.mozilla.org/ja/docs/Web/API/Document/createElement)）
  ```javascript
  const element = document.createElement(tagName)
  ```
* 親Nodeの子Nodeリストの最後にNodeを追加する（[MDN](https://developer.mozilla.org/ja/docs/Web/API/Node/appendChild)）
  ```javascript
  const childElement = parentElement.appendChild(childElement)
  ```
* card、avatar、name、descriptionを連想する変数名を付けるのがよいでしょう。

## Step 2: スタイルとテキストを適用する

下記のような内容となるようにスタイルとテキストを適用してください。
スタイルはあらかじめ定義済みです。
...
<body>
  <div id="app">
    <div class="card">
      ...
      <p class="description">Description Description ...略</p>
    </div>
  </div>
</body>
```
...
* 要素にCSSセレクターを取得/適用する（[MDN](https://developer.mozilla.org/ja/docs/Web/API/Element/className)）
  ```javascript
  elementNodeReference.className = cName   // cNameは文字列
  ```
* 要素にテキストコンテントを取得/適用する（[MDN](https://developer.mozilla.org/ja/docs/Web/API/Node/textContent)）
  ```javascript
  elementNodeReference.textContent = text  // textは文字列
  ```